...
public class LocationService {
	...
	public LocationClientOption getDefaultLocationClientOption(){
		if(mOption == null){
			...
			mOption.setCoorType(COORD_BD09LL);//可选，默认gcj02，设置返回的定位结果坐标系，如果配合百度地图使用，建议设置为bd09ll;
...
//		    mOption.setIsNeedLocationDescribe(true);//可选，默认false，设置是否需要位置语义化结果，可以在BDLocation.getLocationDescribe里得到，结果类似于“在北京天安门附近”
	...
	public void getCurrentPosition(final CallbackContext callback) {
		listener = new BDLocationListener() {
			@Override
			public void onReceiveLocation(BDLocation location) {
				JSONArray message = new ResultBuilder(location).build();
	...
	public boolean watchPosition(final CallbackContext callback) {
		listener = new BDLocationListener() {
			@Override
			public void onReceiveLocation(BDLocation location) {
				JSONArray message = new ResultBuilder(location).build();
